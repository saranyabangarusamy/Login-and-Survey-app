...
const QuestionCreateType = new GraphQLInputObjectType({
    ...
    fields: {
            ...
            is_right: {type: new GraphQLNonNull(GraphQLString)}            
    }
});
...
const UserMutationType = new GraphQLObjectType({
    ...
    fields: {
        ...
        addQuestion: {
            ...
            resolve: (source, { input }) => {    
                return new Promise((resolve, reject) => {  
                    database.run('INSERT INTO Questions (question) VALUES (?)', [input.question], function(err) {                                               
                        if(err) {                            
                            reject(err)
                        }
                        else{
                            database.run('INSERT INTO Options (qid,option,is_right) VALUES (?,?,?),(?,?,?),(?,?,?),(?,?,?)',
                             [this.lastID,input.option1,('option1' === input.is_right),
                                this.lastID,input.option2,('option2' === input.is_right),
                                this.lastID,input.option3,('option3' === input.is_right),
                                this.lastID,input.option4,('option4' === input.is_right)], function(err) {                                
                                if(err) {                                    
                                    reject(err)
                                }
                                else{
